...
## EFLAGS 与操作系统
在[[保护模式]]下，32位的`EFLAGS`寄存器中，除了上述的状态标志，有几个控制位对[[操作系统]]的运行至关重要。

### IF (Interrupt Flag) - 中断允许标志
- **作用**: `IF`位控制[[中央处理器 (CPU)]]是否响应**可屏蔽硬件中断**（如来自[[可编程中断控制器 (PIC) 8259A|8259A]]的`INTR`信号）。
- **`IF = 1`**: 开中断。[[中央处理器 (CPU)]]会正常响应外部硬件的中断请求。
- **`IF = 0`**: 关中断。[[中央处理器 (CPU)]]会暂时忽略所有可屏蔽硬件中断。
- **重要性**:
    - [[操作系统]]在执行[[临界区]]代码时，会先通过`cli`指令（Clear Interrupt Flag）将`IF`置0，以防止被中断打断，保证[[原子操作]]。
    - 在[[中断处理程序]]的入口处，如果通过[[中断门]]进入，`IF`位会被硬件自动清0，防止中断嵌套。
    - **指令**:
        - `sti` (Set Interrupt Flag): 将`IF`置1。
        - `cli` (Clear Interrupt Flag): 将`IF`置0。
- **注意**: `IF`位不影响[[不可屏蔽中断 (NMI)]]和[[异常 (Exception)]]的发生。

### IOPL (I/O Privilege Level) - I/O特权级
- **作用**: `IOPL`是一个2位的字段，它定义了可以执行`in`, `out`等I/O指令所需要的最低[[特权级]]。
- **规则**: 只有当当前[[特权级]]`CPL <= IOPL`时，程序才能无条件执行I/O指令。
- **应用**: [[操作系统]]内核通常将`IOPL`设置为0，这意味着只有Ring 0的代码才能直接访问硬件端口。Ring 3的用户程序如果尝试访问端口，会触发一个`#GP`（General Protection）异常。

### NT (Nested Task Flag) - 任务嵌套标志
- **作用**: `NT`位用于支持硬件层面的任务切换。它标识了当前任务是否是“嵌套”在前一个任务中的。
- **`NT = 1`**: 表示当前任务是被另一个任务通过`call`指令或任务门调用的。其[[TSS]]中保存了指向前一个任务的链接。
- **`NT = 0`**: 表示当前任务不是嵌套任务。
- **与`iret`的交互**:
    - 当`iret`指令执行时，[[中央处理器 (CPU)]]会检查`NT`位。
    - 如果`NT=1`，`iret`会执行一个**任务切换**，返回到前一个任务。
    - 如果`NT=0`，`iret`执行一个普通的**中断返回**，从栈中恢复`CS`, `EIP`, `EFLAGS`。
- **我们的系统**: 尽管我们不使用硬件任务切换，但理解`NT`位对于理解`iret`指令的完整行为至关重要。[[中断处理程序]]开始时，`NT`位会被硬件清0。

### TF (Trap Flag) - 陷阱标志
- **作用**: 当`TF`位置1时，[[中央处理器 (CPU)]]在每执行完一条指令后，都会产生一个**单步中断**（`#DB`，Debug Exception）。
- **应用**: 这是调试器（如gdb）实现单步执行功能的基础。调试器通过修改`EFLAGS`中的`TF`位来控制单步中断的产生。
